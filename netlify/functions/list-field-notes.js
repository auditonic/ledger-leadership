/**
 * List Field Notes Function
 * Public endpoint for listing published field notes
 */
...
export async function handler(event) {
  ...
  if (!resp.ok) {
    ...
    return {
      statusCode: 502,
      headers,
      body: JSON.stringify({ error: "Supabase query failed", details: text }),
    };
  }

  const rows = await resp.json();

  // Strip private notes defensively
  const safe = (rows || [])
    .filter((r) => r.attribution !== "private")
    .map((r) => ({
  ...
  return {
    statusCode: 200,
    headers,
    body: JSON.stringify({ ok: true, notes: safe }),
  };
}
